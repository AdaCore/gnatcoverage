...
* The ``coverage_settings.mk`` secondary makefile, which holds most of the
  ...
  are not all defined in the project file. Because it is created once, meant to
  be adapted by the user, and preserved across regenerations, it is a good
  candidate for version control; see :ref:`Harness_Structure` for an overview
  of all the generated artifacts and which ones to put under version control.
