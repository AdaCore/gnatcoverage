...
expected_diags = [
    HarnessDiagnostic(text="Missing expected sNoCov mark at line 3")
]
if not thistest.options.qualif_level:
    expected_diags.append(
        HarnessDiagnostic(text="Missing expected lNoCov mark at line 3")
    )

HarnessTestCase(expected_diags=expected_diags, category=CAT.stmt).run()
thistest.result()
