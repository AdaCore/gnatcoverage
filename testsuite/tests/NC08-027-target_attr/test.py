...
gpr = gprfor(mains=[], srcdirs=['..'],
             extra='for Target use "foo-bar";\n')
runargs = ['-P', gpr, program_path]
p = xrun(runargs, register_failure=False,
         auto_config_args=False,
         auto_target_args=False)

if thistest.bits() == 32:
    thistest.fail_if(
        not re.search('No builtin or GNATemulator execution driver found for'
                      ' target: foo-bar', p.out),
