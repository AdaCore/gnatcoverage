...
from SUITE.tutils import (exepath_to, gprbuild, gprfor, tracename_for,
                          unixpath_to, xcov, xrun)
...
p = xcov(['coverage', '--level=branch', '--annotate=asm',
          tracename_for('noop')],
         out='not_found.out', register_failure=False)
thistest.fail_if(
    p.status == 0
    or not match(
...
xcov(['coverage', '--level=branch', '--annotate=asm',
      '--exec=nop', tracename_for('noop')],
     out='noop.stdout')
thistest.fail_if(not match('main|noop [+!]:', 'noop.stdout'),
