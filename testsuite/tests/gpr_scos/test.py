...
def trycov(
    ...
    sco_args = [
        "--{}=obj/{}.{}".format(
            "sid" if src_trace else "scos", unit, "sid" if src_trace else "ali"
        )
...
# --scos/--sid alone
trycov(
    sco_units=["inc"], with_project=False, expected_reports=["inc.adb.xcov"]
)
...
trycov(
    sco_units=[],
    with_project=True,
    expected_reports=["inc.adb.xcov", "dec.adb.xcov", "test_ops.adb.xcov"],
)

# --scos with -P, check that --scos prevails
trycov(sco_units=["dec"], with_project=True, expected_reports=["dec.adb.xcov"])
