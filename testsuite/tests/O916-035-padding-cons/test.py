...
from SUITE.tutils import (exepath_to, gprfor, gprbuild, tracename_for, xcov,
                          xrun)
...
insns_x86 = [
    ...
    # 5-bytes padding
    (True,  '# NOP DWORD ptr [EAX + EAX*1 + 00H]\n'
    ...
    (True,  '# 66 NOP DWORD ptr [EAX + EAX*1 + 00H]\n'
...
def run_test(platform, insns):
    ...
    def build_material(name, padding_insn=None):
        ...
        project = gprfor([asm_filename], srcdirs=['.'], objdir='.',
                         langs=['Asm'])
    ...
    for i, ((is_padding, insn), trace_file) in enumerate(zip(insns, traces)):
        args = ['coverage', '--level=insn', '--annotate=asm',
                '--routines={}'.format(routine),
                trace_ref, trace_file]
        ...
        # matching.
        thistest.fail_if(
            consolidation_failed and not
            match('TRACES_NAMES.CONSOLIDATION_ERROR', out_file),
