...
def src_cov(cov_level: str, annotate_level: str, line_coverage: str) -> None:
    try:
        ...
        if annotate_level.find("xcov") != -1:
            thistest.fail_if(
                not match(
                    "[0-9]+ " + line_coverage + line, "engines.adb.xcov"
                ),
                "check " + cov_level + " coverage results on engines example",
            )
    except FatalError:
        pass
...
def obj_cov(cov_level: str, annotate_level: str, line_coverage: str) -> None:
    ...
    xcov(
        [
            ...
            "--level=" + cov_level,
            "--annotate=" + annotate_level,
            "--routines={}".format(symbol),
            tracename_for("test_engines"),
        ]
    )
    if annotate_level.find("xcov") != -1:
        thistest.fail_if(
            not match("[0-9]+ " + line_coverage + line, "engines.adb.xcov"),
            "check " + cov_level + " coverage results on engines example",
        )
