...
sources. gnatcov used to compute them from source file full names: a project
depending on an installed (externally built) instrumented library recomputed
the library buffer symbols from the installed source full names, which do not
match the symbols the library actually defines (computed from the original
source full names when the library was instrumented), leading to an undefined
symbol error at link time.

Buffer symbols for file-based languages are now derived from the name of the
project that owns the source and from the source simple name, both of which
installation preserves. Check the three features that reference those
symbols: the automatic buffer dump, the manual buffer dump/reset (buffer
control annotations) and the live observability API.
"""
...
# list unit of the main project must name buffer symbols that the installed
# library defines.
...
# trace must contain the coverage data of the library, and the reset must
# clear the library buffers, so the "t2" trace must report the library
# function as not covered.
...
# generated Sum_Buffer_Bits function goes through the main project's buffers
