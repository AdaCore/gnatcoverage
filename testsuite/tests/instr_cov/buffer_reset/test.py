...
from SCOV.minicheck import CovReport, build_and_run, check_xcov_reports, xcov
...
def get_expected_cov(trace_name: str) -> CovReport:
    """
    Generate an expected coverage results based on the prefix of the trace
    indicating which main was executed, and its index.
    """
    m = re.match(pattern=r"(.*)-(\d)\.srctrace", string=trace_name)
    thistest.fail_if(
        not m, comment="trace name not in expected format: " + trace_name
    )
    assert m
    trace_lang = m.group(1)
    idx = int(m.group(2))
    expected_cov = {}
...
def check_one_exec(cov_args: list[str], lang: str) -> None:
    """
