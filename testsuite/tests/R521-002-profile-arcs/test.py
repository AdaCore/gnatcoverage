...
gprbuild(gpr, extracargs='-fprofile-arcs',
         largs='-fprofile-arcs -ftest-coverage')

xrun(main)
xcov(['coverage', '-P', gpr, '-cstmt+decision', '-axml', main_trace],
     out='coverage.log')

warning_template = (r'warning: .*{}: unit compiled with instrumentation'
                    r' \(-fprofile-arcs\)')

for unit in ('main.ali', 'util.c.gli'):
    thistest.fail_if(
        not any(re.match(warning_template.format(re.escape(unit)), line)
