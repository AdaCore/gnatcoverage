...
"""
Example script to be used as a testsuite.py --gnatcov-run argument, which the
suite engine will call instead of "gnatcov run" to execute programs and produce
an execution trace that "gnatcov coverage" can process later on.
...
interface, at least the subset of arguments that the testsuite driver might use
for your target configuration. Simplest is to error out on unrecognized
options.

The code below eventually resorts to a real "gnatcov run" invocation. While
there is no real added value in doing that, this code is useful as an example
of a possible way to achieve the goal of replacing gnatcov run in other
situations. A typical case is one where the program execution is performed on a
real target board, producing a some trace that needs to be converted to a
