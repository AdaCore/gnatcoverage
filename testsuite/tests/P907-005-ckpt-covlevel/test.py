...
def create_checkpoint(name, covlevel):
    ...
    prj_file = gprfor(mains=['test_{}.adb'.format(name)],
                      prjid=name,
...
args = ['coverage', '--annotate=xcov', '--output-dir=output', '-C', add_ckpt,
        '-C', mult_ckpt]
...
p = xcov(args + ['--level=stmt+decision'], 'cons-stmt-decision.log',
         register_failure=False)
thistest.fail_if(
    p.status == 0 or not re.match(
