...
def check_error(argv, log_name, expected_error):
    ...
    thistest.fail_if(p.status == 0,
                     'gnatcov was expected to fail, it did not')
    thistest.fail_if(
        ...
        'but got:\n'
        '{}'.format(log_name,
                    indent(repr(expected_re.pattern)),
                    indent(actual_error)))
...
# SID file.
check_error(argv=['coverage', '--annotate=xcov', '--level=stmt+mcdc',
                  '--sid', ckpt(1), trace(1)],
            log_name='ckpt_as_sid.log',
            expected_error=('invalid Source Instrumentation Data (SID) file '
                            '{}, name of file should have .sid extension'
                            .format(ckpt(1))))

# Check that gnatcov complains when passing an SID file as a consolidation
# checkpoint.
check_error(argv=['coverage', '--level=stmt+mcdc',
                  '--checkpoint', sid(1),
                  '--checkpoint', sid(2),
                  '--save-checkpoint', 'consolidated.ckpt'],
...
# enough.
xcov(['coverage', '-v', '--annotate=xcov', '--level=stmt+mcdc',
