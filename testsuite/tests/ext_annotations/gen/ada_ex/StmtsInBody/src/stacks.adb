      ...
      case Op is -- # op_case
         when Push =>
            pragma Annotate                      -- # op_oflow
              (Xxxx, Exempt_On, "no overflow");  -- # op_oflow
            if S.Vcount = S.Size then            -- # op_oflow_v1
               raise Constraint_Error;           -- # op_oflow_v2
            end if;                              -- # op_oflow
            pragma Annotate (Xxxx, Exempt_Off);  -- # op_oflow
         ...
         when Pop =>
            pragma Annotate                      -- # op_uflow
              (Xxxx, Exempt_On, "no underflow"); -- # op_uflow
            if S.Vcount = 0 then                 -- # op_uflow_v1
               raise Constraint_Error;           -- # op_uflow_v2
            end if;                              -- # op_uflow
            pragma Annotate (Xxxx, Exempt_Off);  -- # op_uflow
