...
gpr = gprfor(srcdirs=['..'], mains=['test_p1.adb',
                                    'test_p2.adb',
                                    'test_p3.adb'])
...
thistest.fail_if(not re.search(string=report_p1, pattern=r'\.:.*pragma'),
                 "pragma not reported as '.' for p1")

report_p2 = contents_of('obj/p2.ads.xcov')
thistest.fail_if(not re.search(string=report_p2, pattern=r'\.:.*pragma'),
                 "pragma not reported as '.' for p2")
thistest.fail_if(not re.search(string=report_p2, pattern=r'\.:.*type'),
                 "type not reported as '.' for p2")

report_p3 = contents_of('obj/p3.ads.xcov')
thistest.fail_if(not re.search(string=report_p3, pattern=r'\.:.*pragma'),
                 "pragma not reported as '.' for p3")
if thistest.options.trace_mode == 'bin':
    thistest.fail_if(not re.search(string=report_p3, pattern=r'\-:.*type'),
