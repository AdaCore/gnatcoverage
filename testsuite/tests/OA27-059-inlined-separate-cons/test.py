...
if (
    thistest.options.trace_mode == 'bin' and
    '-O1' in thistest.suite_cargs_for('Ada')
):
    p = xcov(['coverage', '-c', covlevel, '--annotate=report', output_arg,
              test1.tracename, test2.tracename])
    thistest.fail_if(
        p.status == 0,
...
checked_xcov(['coverage', '-c', covlevel, '--annotate=xcov', output_arg,
              '-C', test1.checkpoint,
              '-C', test2.checkpoint],
             'cons.log')
