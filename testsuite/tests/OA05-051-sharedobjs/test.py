...
from SUITE.tracelib import (
    TraceEntry, TraceSpecial, TraceFile, TraceOp, TraceKind,
    create_exec_infos, create_trace_header)
...
def get_symbol(filename, symbol):
    ...
    raise ValueError('Could not find symbol {} in {}'.format(
        symbol, filename
    ))


ELFHeader = struct.Struct(
    '<16s'  # e_ident
    'H'    # e_type
    'H'    # e_machine
)
...
with open(myprog_filename, 'rb') as f:
    ...
    machine = (((e_machine & 0xff) << 8) | (e_machine >> 8)
               if big_endian else
               e_machine)
...
load_events = {
    libname: TraceEntry(
        bits, so_load_addr,
        TraceSpecial.LoadSharedObject, TraceOp.Special,
        create_exec_infos(libfiles[libname], code_size=sym.last + 1)
    )
    for libname, sym in syms.items()
}
unload_event = TraceEntry(bits, so_load_addr, TraceSpecial.UnloadSharedObject,
                          TraceOp.Special)
...
def write_traces(trace_filename, executed_libs):
    ...
    for libname in libnames:
        ...
        if libname in executed_libs:
            sym = syms[libname]
            entries.append(TraceEntry(bits, so_load_addr + sym.addr,
                                      sym.size,
                                      TraceOp.Block))
...
for tag, exp in expectations.items():
    ...
    xcov(['coverage', '-Pmyprog', '--level=stmt', '--annotate=xcov',
          '--output-dir', output_dir, in_tmp('myprog_{}.trace'.format(tag))])
