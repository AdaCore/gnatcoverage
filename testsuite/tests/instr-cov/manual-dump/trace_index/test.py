...
thistest.fail_if(
    len(cov_args) < 4,
    comment=f"expected at least three traces: {cov_args}"
)

traces = glob.glob(srctrace_pattern_for("main", manual=True, manual_prj_name="p"))
traces.sort()

thistest.fail_if(
    len(traces) != 3,
    comment=f"Expected three traces, got {len(traces)}"
)
