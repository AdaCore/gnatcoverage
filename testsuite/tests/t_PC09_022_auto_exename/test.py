...
class Project:
    ...
    def run(self) -> None:
        ...
        thistest.stop_if(
            ...
            FatalError(
                "Test declaration disagrees with test framework about"
        ...
        if error_msg_re:
            ...
            thistest.fail_if(
                ...
                "\n{}".format(
                    self.name, log_file, indent(out), indent(error_msg_re)
                ),
            )
...
for p in (
    ...
    Project(project_files, "nomain", "src", should_succeed=False),
