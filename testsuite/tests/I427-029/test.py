...
from SUITE.tutils import (exepath_to, gprbuild, gprfor, maybe_valgrind,
...
gprbuild(gprfor([Vtest('test_engines', v).filename('.adb') for v in variants],
                srcdirs='../src'))
...
for variant in variants:
    ...
    xcov(
        ['dump-trace', tracename_for(vt.filename())],
        vt.filename('.tracedump'))
    thistest.fail_if(
        not match('Tag.*(User_Tag).*\nLen.*\nData.*%s' % vt.tag(),
                  vt.filename('.tracedump')),
...
xcov(['coverage', '--level=branch', '--annotate=xcov',
     ...
     + [tracename_for(Vtest('test_engines', v).filename())
        for v in variants])
thistest.fail_if(
    not match(r' \+:.*return.*Stable_P', 'engines.adb.xcov'),
...
xcov(['coverage', '--level=branch', '--annotate=html+',
      '--routines=@list.stable']
     + [Vtest('test_engines', v).filename(tracename_for(''))
        for v in variants])
