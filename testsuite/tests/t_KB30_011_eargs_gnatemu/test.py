...
def gnatemulator(args: str, exe: str) -> str:
    ...
    Run(
        to_list(
            "%(target)s-gnatemu %(args)s %(kernel)s %(exe)s"
            % {
...
output = gnatcovrun(
    register_failure=not is_ignored_target, extra=extra, exe=exe
)
...
eargs2 = "-version"
output = gnatemulator(
    "-v --eargs %s --eargs-end --eargs %s --eargs-end" % (eargs1, eargs2), exe
)
thistest.fail_if(
