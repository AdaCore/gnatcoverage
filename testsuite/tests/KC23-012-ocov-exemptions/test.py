...
base_coverage = ['coverage', '--level=branch', '--annotate=xcov',
                 tracename_for('p')]
...
def check(ali_spec):
    ...
    thistest.fail_if(
        not re.search(pattern='%s:.*# expect-never-true' % xnote_never_true,
                      string=this_report),
        '%s, unmatched expected-never-true' % hint)
    thistest.fail_if(
        not re.search(pattern='%s:.*# expect-uncovered' % xnote_uncovered,
                      string=this_report),
