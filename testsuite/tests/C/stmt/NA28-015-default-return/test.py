...
def run_trace_with_args(prefix, exec_args):
    build_and_run(
        ...
        extra_instr_args=[f"--dump-filename-prefix={prefix}",
...
def trycov(cases, xnocov):
    ...
    fun_tracename = (srctracename_for if thistest.options.trace_mode == 'src'
                     else tracename_for)

    xcov('coverage --level=stmt --annotate=xcov'
         ' -P gen.gpr %s'
         % ' '.join(fun_tracename(c) for c in cases))
    ...
    def check_mark_on(line):
        ...
        thistest.fail_if(mark in xnocov and note != '-',
                         "missing expected '-' note on line:\n%s" % line)

        thistest.fail_if(note == '-' and mark not in xnocov,
