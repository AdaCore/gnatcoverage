"""
Test initializer range for integrated instrumentation for bareboard targets.
"""
...
gcvrt_prefix = "gcvrt"
xcov(["setup", f"--prefix={gcvrt_prefix}", "--restricted-to-languages=C,C++"])
env.add_search_path(
    "GPR_PROJECT_PATH",
...
with open(main_prj, "w") as prj_file:
    ...
    print('   for Main use ("main.cpp");', file=prj_file)
    print('   for Object_Dir use "obj";', file=prj_file)
    print("   package Compiler is", file=prj_file)
    print(
        f"""
      for Switches ("C++") use {bsp_prj_name}.Common_Switches;
    """,
        file=prj_file,
    )
    print("   end Compiler;", file=prj_file)
...
xcov(
    [
        "setup-integration",
        "--level=stmt+mcdc",
        f"--files={os.path.abspath('main.cpp')}",
...
xcov(
    [
        "coverage",
        "--sid",
        os.path.join(gnatcov_artifact_dir, "main.cpp.sid"),
        "-axcov",
        "-cstmt+mcdc",
        trace_name,
    ]
...
check_xcov_reports(
    reports_dir=".",
    expected_cov={"main.cpp.xcov": {"+": {6, 7, 9, 11}}},
)
