...
gprbuild(gprfor(mains=['test_min.adb', 'test_min1.adb', 'test_min2.adb'],
                srcdirs='..'),
         gargs=['-O1'])
...
# get full coverage
xcov('coverage --level=branch --annotate=asm ' + tracename_for('test_min'),
     out='test_min.out')

thistest.fail_if(not match(r'_ada_min \+:.*', 'test_min.out'),
...
xcov('coverage -c branch -a asm --routines=_ada_min '
...
xcov('coverage --level=branch --annotate=asm --routines=@../func.list '
     + tracename_for('test_min2'),
     out='test_min2.out')
thistest.fail_if(not match(alt_sign[dir1] + ': .* ' + insn1, 'test_min2.out'),
