...
p = Run(cmd)
thistest.fail_if(
    p.status != 0, "command '%s' failed ; output was:\n%s" % (
        ' '.join(cmd), p.out))
...
thistest.fail_if(
    not trace.first_header, "info header missing from trace file")

thistest.fail_if(
    not trace.infos, "info entries missing from trace file")
...
for info_entry in trace_info.values():
    ...
    if info_entry.kind in expected_entries:
        ...
        thistest.fail_if(
            ...
            "value was '%s'" % (expected_contents, info_entry.kind,
                                info_entry.data))
