...
run_and_check(
    xcov_args + ["-axcov", "--dump-units-to=foo1.txt"], "xcov-foo.txt", ""
)
...
run_and_check(
    xcov_args + ["-areport", "--dump-units-to=-"],
    "report-stdout.txt",
    REPORT_PATTERN,
    regexp=True,
)
...
run_and_check(
    xcov_args
    + ["-areport", "--dump-units-to=foo2.txt", "--output=report2.txt"],
...
run_and_check(
    xcov_args
    + ["--units=main", "--dump-units-to=-", "--save-checkpoint=c1.ckpt"],
    "ckpt-1.txt",
    "main\n",
)
run_and_check(
    xcov_args + ["--units=pkg", "--save-checkpoint=c2.ckpt"], "ckpt-2.txt", ""
)
...
run_and_check(
    xcov_args + ["--save-checkpoint=c3.ckpt", scos_arg], "ckpt-3.txt", ""
)
run_and_check(
