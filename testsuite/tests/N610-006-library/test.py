...
prj = gprfor(['test_inc.adb'],
             srcdirs=['..'],
             objdir='obj',
             deps=['../libops'])

xcov_args = build_and_run(gprsw=GPRswitches(root_project=prj),
                          covlevel='stmt',
                          mains=['test_inc'],
                          extra_coverage_args=[])
...
def trycov(verbose, annotate):
    ...
    nocode_notified = re.search(pattern='no object code for mult.ali',
                                string=p.out)

    bin_mode = thistest.options.trace_mode == 'bin'

    if bin_mode and verbose and not nocode_notified:
        thistest.failed('missing notice of absence of object code,'
                        ' verbose=%s, annotate=%s' % (verbose, annotate))

    if bin_mode and nocode_notified and not verbose:
        thistest.failed('unexpected notice of absence of object code,'
                        ' verbose=%s, annotate=%s' % (verbose, annotate))

    if annotate == 'report':
        thistest.fail_if(
            not re.search(pattern='mult.adb.*: statement not executed',
                          string=p.out),
            'missing expected stmt violation for mult.adb in report')

    if annotate == 'xcov':
        thistest.fail_if(
            not re.search(pattern='[0-9]+ -:',
                          string=contents_of('mult.adb.xcov')),
