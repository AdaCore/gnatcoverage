...
def check(tc_name, coverage_args, expected_results, expect_failure):
    p = xcov(['coverage', '-P', prj, '-axcov', '-cstmt'] + coverage_args,
             out=f"{tc_name}.log", register_failure=not expect_failure)
    if expect_failure:
        thistest.fail_if(p.status == 0,
                         "gnatcov did not exit with an error ({tc_name})")

    actual = '\n'.join(line.rstrip()
                       for line in contents_of(f"{tc_name}.log").splitlines())

    thistest.fail_if_no_match(f"unexpected gnatcov output ({tc_name})",
...
bin_trace = build_and_run(
    ...
    extra_coverage_args=[],
    mains=['main'],
    extra_gprbuild_args=['-XBUILD_MODE=bin-traces',
                         '-cargs', '-g', '-fdump-scos',
...
check("mix_not_allowed",
      coverage_args=['--scos', 'obj/main.ali',
                     '--sid', 'obj/main.sid',
                     src_trace, bin_trace],
...
check("scos_with_src",
      coverage_args=['--scos', 'obj/main.ali', '--sid', 'obj/main.sid',
