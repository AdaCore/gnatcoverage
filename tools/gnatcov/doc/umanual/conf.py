...
# Tool version & release, fetched from the source files

with open(os.path.join('..', '..', 'version.ads'), 'r') as f:
    content = f.read()
match = re.match(r'.*Version.*"(.*)".*', content, re.DOTALL)
if not match:
    raise ValueError('Could not fetch the version & release from version.ads')

version = match.group(1)
release = "%s %s" % (project, version)
...
html_short_title = "%s %s" % (project, docname)
...
with open ("latex_preamble.tex") as f:
    custom_latex_preamble = f.read ()
...
epub_title = u'GNATcoverage'
epub_author = u'AdaCore'
epub_publisher = u'AdaCore'
epub_copyright = u'2009-2011, AdaCore'
