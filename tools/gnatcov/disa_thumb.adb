...
package body Disa_Thumb is
   ...
   function To_Insn16 (Insn_Bin : Binary_Content) return Unsigned_16
   is (Disa_Common.ELF_To_U16
         (Slice (Insn_Bin, Insn_Bin.First, Insn_Bin.First + 1)));

   function To_Insn32 (Insn_Bin : Binary_Content) return Unsigned_32
   is (Unsigned_32
         (Disa_Common.ELF_To_U16
            (Slice (Insn_Bin, Insn_Bin.First + 2, Insn_Bin.First + 3)))
       + Shift_Left
           (Unsigned_32
              (Disa_Common.ELF_To_U16
                 (Slice (Insn_Bin, Insn_Bin.First + 0, Insn_Bin.First + 1))),
            16));

   function Is_32bit_Insn (Insn_Bin : Binary_Content) return Boolean
   is (case Shift_Right (To_Insn16 (Insn_Bin), 11) is
         when 2#00000# .. 2#11100# => False,
         when others               => True);

   function Slice
     (U32 : Unsigned_32; Shift : Natural; Mask : Unsigned_32)
      return Unsigned_32
   is (Shift_Right (U32, Shift) and Mask);

   function Sign_Extend (U32 : Unsigned_32; Width : Natural) return Unsigned_32
   is (Shift_Right_Arithmetic (Shift_Left (U32, 32 - Width), 32 - Width));

   function Get_Target
     (Immediate : Unsigned_32; Shift : Natural; Pc : Unsigned_32)
      return Unsigned_32
   ...
   overriding
   procedure Initialize (Object : in out Thumb_Disassembler) is
   begin
   ...
   overriding
   procedure Finalize (Object : in out Thumb_Disassembler) is
   begin
   ...
   function Get_Insn_Length
     (Self : Thumb_Disassembler; Insn_Bin : Binary_Content) return Positive
   is
            ...
            case Opcode is
               ...
               when 2#01#  =>
                  if (Insn32 and 16#fff0_ffe0#) = 16#e8d0_f000# then
                     ...
                     if (Insn32 and 2**15) = 2**15 then
                        ...
                        --  This instruction writes in PC, so it's a
               ...
               when 2#10#  =>
                  declare
                     ...
                     Imm6  : constant Unsigned_32 :=
                       Slice (Insn32, 16, 16#3f#);
                     Imm10 : constant Unsigned_32 :=
                       Slice (Insn32, 16, 16#3ff#);
                     Imm11 : constant Unsigned_32 :=
                       Slice (Insn32, 0, 16#7ff#);
                  ...
                  if (Insn16 and 16#0500#) = 16#0100# then
                     ...
                     declare
                        ...
                        Immediate : constant Unsigned_32 :=
                          (Shift_Left (Imm5, 1) or Shift_Left (I, 6));
                     begin
                        Branch_Dest.Target :=
                          Pc_Type (Get_Target (Immediate, 0, PC32));
                     end;
   ...
   function Is_Padding
     (Self : Thumb_Disassembler; Insn_Bin : Binary_Content; Pc : Pc_Type)
      return Boolean
   is
