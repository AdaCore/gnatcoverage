...
typedef struct gnatcov_rts_unit_coverage_buffers
{
  /* Hash of SCO info for this unit, as gnatcov computes it (see
     ...
     and coverage data are consistent. Specific hash values are computed during
     instrumentation.  */
  ...
  /* Index for the last bits in coverage buffers for statements, decisions and
     MC/DC.  */
...
/* MCDC_PATH_ADDRESS is the address of a local variable storing the mcdc_state.
   If FIRST is true, first reset it to 0.  If VALUE is true, add
   OFFSET_FOR_TRUE.  */
extern gnatcov_rts_bool
