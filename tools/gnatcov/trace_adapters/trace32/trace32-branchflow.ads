...
package Trace32.Branchflow is
   ...
   type Status_Kind is
     (Status_Ok, File_Error, Invalid_Trace_File, No_More_Entry);

   function Open
     (This : in out Branchflow_Trace; Path : String) return Status_Kind
   with
     Pre  => not This.Is_Open,
     Post =>
       Open'Result in Status_Ok | File_Error | Invalid_Trace_File
       and then (if Open'Result = Status_Ok then This.Is_Open);
   ...
   with Pre => This.Is_Open, Post => not This.Is_Open;
