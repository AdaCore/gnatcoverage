...
that give a hard limit on test set size for various coverage metries.
...
The following will not re-define what has already detailed
...
in~\cite{bryant86:graphs}.  In a nutshell, BDDs can be described as an
...
The first is the easiest one and we will let it as an exercise for the
...
In \cite{rcdc-from}, it is proven that strongly coupled conditions
...
decision. e.g. if $SUBDEC=(B \ \adaor{} \ C)$, the following evaluation pairs
proves Masking MC/DC, but not Unique Cause MC/DC:
...
paths, which would contract the hypothesis. This means that any
...
one of the two properties on the BDD is violated and let us show
...
The proof of this theorem is still to be produced. We have however
used the Alloy model \verb|bdd_dec_coverage.als| with up to 4
conditions in decisions; which already gives a good hint of the
validity of this conjecture.

\subsection{Coupled conditions without diamonds}

First, it can be seen that strong coupling only gives degredated case,
...
\begin{theorem}
  ...
  A BDD with no diamond and with strongly coupled conditions cannot be
...
In a BDD with no diamond, there is only one possible path to each
...
Or a even more common pattern, that can often be found in C:
...
cases ($t < T$, $t = T$, $t > T$) using three conditions that are such
