...
package Qemu_Traces is
   ...
   --  file, possibly controlled by an internal input "Trace Control" file for
   --  mcdc like coverage criteria.

   --  The Trace Control internal input contains a list of addresses ranges
   --  for which branch history is needed, computed by xcov from SCO decision
   --  entries and passed to the simulation engine.  We refer to this Trace
   --  Control simulation input as a Decision Map.

   --  The Execution Trace output contains a list of execution trace entries
   --  generated by the simulation engine, and preceded by a list of trace
