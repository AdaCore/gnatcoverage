...
def print_warn_stats():
    for key in warn_stats:
        logger.log_info("%s: %d" % (key, warn_stats[key]))
...
class Dir(Artifact):
    def __init__(
        ...
        # entry point.
        if os.path.relpath(self.root, art_root) == ".":
...
"""
    ...
    def rest_doc_ref(self: Dir, id_as_title: bool = False) -> str:
        ...
        return writer.qmref(
            self.docfile(True),
            self.title() if not id_as_title else self.id,
        )
    ...
    def parent_ref(self: Dir) -> str:
        """Return link to the parent object, if self's parent is a
        requirement(set) or testcase(set)

        Include artifact type when parent is requirement.
        """
        if self.pdo and (
            is_req_or_set(self.pdo)
            or is_tc_or_set(self.pdo)
            or self.pdo.tcgroup
            or self.pdo.reqgroup
        ):
            return (
                "**Parent"
...
class DirTree_FromPath(DirTree):
    ...
    def __init__(self: DirTree_FromPath, rootp: str, dolevel: str):
    ...
    def topdown_map(
        ...
        excluded_levels = {"doA", "doB", "doC"}
        excluded_levels.remove(self.dolevel)
...
class DocGenerator(object):
    def __init__(
        ...
        logger.log_debug(f"Root testsuite dir: {self.art_rootdir}")
    ...
    def art_dirtree_for(
        ...
        dirtree = DirTree_FromPath(
            rootp=self.art_rootdir, dolevel=self.o.dolevel
        )
...
if __name__ == "__main__":
    ...
    op.add_option(
        ...
        dest="testsuite_dir",
        default=None,
        help="Root directory of the qualification testsuite",
    )
    ...
    logger.current_level = (
        logger.Log_Level_Kind.DEBUG
        if verbose
        else logger.Log_Level_Kind.WARNING
    )

    actual_root_dir = (
        options.testsuite_dir if options.testsuite_dir else ART_ROOTDIR
    )
    if not actual_root_dir.endswith("Qualif"):
        actual_root_dir = os.path.join(actual_root_dir, "Qualif")

    mygen = DocGenerator(
        art_rootdir=actual_root_dir, rst_rootdir=RST_ROOTDIR, options=options
    )
