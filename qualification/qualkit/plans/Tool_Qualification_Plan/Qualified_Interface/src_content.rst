...
* Instrument the application for coverage assessment and test harness main
  ...
  Specify the coverage criterion level to be assessed as specified in the
  *Operational Environment* section of the |tor_abb| document. A trace
  <DUMP_METHOD> and <DUMP_CHANNEL> must also be specified, through the
  :literal:`--dump-trigger=<DUMP_METHOD>` and
  :literal:`--dump-channel=<DUMP_CHANNEL>` switches, but the specific values
  only need to be in accordance to the restrictions imposed by the GNATPro
  runtime library profile and execution host capabilities, defined in the
  *Operational Environment* section of the |tor_abb| document, as described
  in the tool User's Manual.

  Optionally add the :literal:`--instrument-block` switch in order to improve
  the performance of the instrumented code, provided the following additional
  restrictions are respected:
  
  * The code shall not contain any exception handlers. This can be enforced
    with the :literal:`pragma Restrictions (No_Exception_Handlers);` pragma;
  * The coverage traces generated by an execution of the program which
    terminates abnormally (due to an unhandled exception, etc) must be
    discarded.

  Also specify the set of units of interest for which coverage should be
  assessed trough the <UNITS> arguments, which should at least include a <ROOT
  PROJECT> indication using the :literal:`-P<ROOT_PROJECT>` switch:

  .. code-block:: text

   gnatcov instrument [--target=<TARGET> --RTS=<RTS>]
      --dump-trigger=<DUMP_TRIGGER> --dump-channel=<DUMP_CHANNEL>
      [--instrument-block] --level=<LVL> -P<ROOT_PROJECT> <UNITS>
...
* <DUMP_TRIGGER> identifies the trigger to produce execution traces, as
  described in the tool User's Manual;
* <DUMP_CHANNEL> identifies the medium used by the instrumented executable to
  output the execution traces, as described in the tool User's Manual;
